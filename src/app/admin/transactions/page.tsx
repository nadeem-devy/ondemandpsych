  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => window.open(`/api/admin/transactions?format=csv&${new URLSearchParams({ ...(search && { search }), ...(statusFilter && { status: statusFilter }) })}`, "_blank")} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 text-xl font-medium hover:bg-white/10">
          <Download size={14} />
          Export CSV
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-5 rounded-2xl bg-[#0D1B4B]/60 border border-white/10">
          ...
          <p className="text-xl text-white/35">Total Revenue</p>
        </div>
        <div className="p-5 rounded-2xl bg-[#0D1B4B]/60 border border-white/10">
          ...
          <p className="text-xl text-white/35">Failed Payments</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search by user or plan..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white text-xl placeholder:text-white/25 focus:outline-none focus:border-[#FDB02F]/30" />
        </div>
        <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border text-xl font-medium ${showFilters ? "bg-[#FDB02F]/10 border-[#FDB02F]/20 text-[#FDB02F]" : "bg-white/5 border-white/10 text-white/50"}`}>
      ...
      <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 overflow-hidden">
          ...
          <tbody>
            ...
            txns.map((t) => (
              <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                <td className="px-4 py-3"><p className="text-white/70">{t.user.name}</p><p className="text-white/30 text-lg">{t.user.email}</p></td>
                ...
                <td className="px-4 py-3"><span className={`inline-flex px-2 py-0.5 rounded-full text-lg font-medium border ${statusColors[t.status] || ""}`}>{t.status}</span></td>
        ...
        {pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/5">
            <p className="text-xl text-white/25">Page {pagination.page} of {pagination.totalPages}</p>
