    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Payment history and billing</p>
        </div>
        <button onClick={() => window.open(`/api/admin/transactions?format=csv&${new URLSearchParams({ ...(search && { search }), ...(statusFilter && { status: statusFilter }) })}`, "_blank")} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 text-xs font-medium hover:bg-white/10">
          <Download size={14} />
          Export CSV
        </button>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search by user or plan..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white text-xs placeholder:text-white/25 focus:outline-none focus:border-[#FDB02F]/30" />
        </div>
        <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border text-xs font-medium ${showFilters ? "bg-[#FDB02F]/10 border-[#FDB02F]/20 text-[#FDB02F]" : "bg-white/5 border-white/10 text-white/50"}`}>
      ...
      {showFilters && (
        <div className="flex items-center gap-3 mb-4 p-3 rounded-lg bg-white/[0.02] border border-white/5">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 text-xs focus:outline-none">
