  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={fetchHealth} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 text-base font-medium hover:bg-white/10 disabled:opacity-50">
        ...
        <div className={`rounded-2xl border p-6 mb-6 ${data.status === "healthy" ? "bg-green-400/5 border-green-400/20" : "bg-red-400/5 border-red-400/20"}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/40 text-base">All services checked</p>
            </div>
          </div>
        </div>
      )}
          ...
          return (
            <div key={name} className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-5">
              <div className="flex items-center justify-between mb-2">
                <p className="text-white text-base font-medium capitalize">{name.replace(/_/g, " ")}</p>
