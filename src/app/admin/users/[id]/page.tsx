    ...
    <div className="p-8">
      ...
      <div className="flex gap-1 mb-6">
        {([
          ...
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-lg font-medium transition-colors ${tab === t.key ? "bg-[#FDB02F]/15 text-[#FDB02F]" : "text-white/40 hover:text-white/60 hover:bg-white/5"}`}>
            <t.icon size={14} />
            {t.label}
          </button>
        ))}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            ...
            <div>
              ...
              <input type="number" value={form.trialMessageLimit} onChange={(e) => setForm({ ...form, trialMessageLimit: parseInt(e.target.value) || 10 })} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg focus:outline-none focus:border-[#FDB02F]/30" />
            </div>
          ...
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            ...
            <textarea
              ...
              rows={6}
              className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg placeholder:text-white/20 focus:outline-none focus:border-[#FDB02F]/30 resize-none"
...
function Field({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      ...
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg placeholder:text-white/20 focus:outline-none focus:border-[#FDB02F]/30" />
    </div>
  );
}
...
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="text-lg text-white/40 mb-1 block">{label}</label>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg focus:outline-none capitalize">
        {options.map((opt) => (
