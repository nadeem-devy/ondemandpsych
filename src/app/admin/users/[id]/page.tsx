    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => handleAction("reset_password")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-sm hover:bg-white/10">Reset Password</button>
          <button onClick={() => handleAction("resend_welcome")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-sm hover:bg-white/10">Resend Welcome</button>
          <button onClick={() => handleAction("send_verification")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-sm hover:bg-white/10">Send Verification</button>
          <button onClick={() => handleAction("reset_trial")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-sm hover:bg-white/10">Reset Trial</button>
          {user.status === "active" ? (
            <button onClick={() => handleAction("suspend")} className="px-3 py-2 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 text-sm hover:bg-orange-500/20">Suspend</button>
          ) : (
            <button onClick={() => handleAction("activate")} className="px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-sm hover:bg-green-500/20">Activate</button>
          )}
          <button onClick={handleDelete} className="flex items-center gap-1 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm hover:bg-red-500/20">
            <Trash2 size={11} />
            Deactivate
          </button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-[#FDB02F] text-[#07123A] text-sm font-bold hover:bg-[#FDAA40] disabled:opacity-50">
            <Save size={11} />
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
      ...
      <div className="flex gap-1 mb-6">
        {([
          ...
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-medium transition-colors ${tab === t.key ? "bg-[#FDB02F]/15 text-[#FDB02F]" : "text-white/40 hover:text-white/60 hover:bg-white/5"}`}>
            <t.icon size={14} />
            {t.label}
          </button>
        ))}
      ...
      {tab === "details" && (
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            <h2 className="text-white font-semibold text-sm mb-2">Profile</h2>
          ...
          {/* Plan & Status */}
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            <h2 className="text-white font-semibold text-sm mb-2">Plan & Status</h2>
            ...
            <div>
              <label className="text-sm text-white/40 mb-1 block">Trial Message Limit</label>
              <input type="number" value={form.trialMessageLimit} onChange={(e) => setForm({ ...form, trialMessageLimit: parseInt(e.target.value) || 10 })} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-[#FDB02F]/30" />
            </div>
            <div className="flex items-center gap-3 pt-2">
              <div className="text-sm text-white/40">Trial Used:</div>
              ...
              <div className="text-sm text-white/30">{user.trialMessageCount}/{user.trialMessageLimit}</div>
            </div>
          </div>

          {/* Compliance */}
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            <h2 className="text-white font-semibold text-sm mb-2">Compliance</h2>
            ...
            <div className="flex items-center gap-3">
              <label className="text-sm text-white/40">License Verified</label>
          ...
          {/* Notes */}
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            <h2 className="text-white font-semibold text-sm mb-2">Admin Notes</h2>
            <textarea
              value={form.notes}
              onChange={(e) => setForm({ ...form, notes: e.target.value })}
              rows={6}
              className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs placeholder:text-white/20 focus:outline-none focus:border-[#FDB02F]/30 resize-none"
      ...
      {tab === "chats" && (
        <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 overflow-hidden">
          {user.chats.length === 0 ? (
            <div className="p-12 text-center text-white/30 text-sm">No chats yet</div>
          ) : (
      ...
      {tab === "audit" && (
        <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 overflow-hidden">
          {auditLogs.length === 0 ? (
            <div className="p-12 text-center text-white/30 text-sm">No audit logs yet</div>
          ) : (
              ...
              {auditLogs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {log.details && (
                      <p className="text-white/20 text-xs mt-0.5 truncate">{log.details}</p>
                    )}
                  </div>
                  <span className="text-xs text-white/20 shrink-0">{new Date(log.createdAt).toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
...
function Field({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      <label className="text-sm text-white/40 mb-1 block">{label}</label>
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs placeholder:text-white/20 focus:outline-none focus:border-[#FDB02F]/30" />
    </div>
  );
}
...
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="text-sm text-white/40 mb-1 block">{label}</label>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs focus:outline-none capitalize">
        {options.map((opt) => (
