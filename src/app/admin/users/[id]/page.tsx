    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => handleAction("reset_password")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-[11px] hover:bg-white/10">Reset Password</button>
          <button onClick={() => handleAction("resend_welcome")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-[11px] hover:bg-white/10">Resend Welcome</button>
          <button onClick={() => handleAction("send_verification")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-[11px] hover:bg-white/10">Send Verification</button>
          <button onClick={() => handleAction("reset_trial")} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white/50 text-[11px] hover:bg-white/10">Reset Trial</button>
          {user.status === "active" ? (
            <button onClick={() => handleAction("suspend")} className="px-3 py-2 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 text-[11px] hover:bg-orange-500/20">Suspend</button>
          ) : (
            <button onClick={() => handleAction("activate")} className="px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-[11px] hover:bg-green-500/20">Activate</button>
          )}
          <button onClick={handleDelete} className="flex items-center gap-1 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-[11px] hover:bg-red-500/20">
            <Trash2 size={11} />
            Deactivate
          </button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-4 py-2 rounded-lg bg-[#FDB02F] text-[#07123A] text-[11px] font-bold hover:bg-[#FDAA40] disabled:opacity-50">
            <Save size={11} />
            {saving ? "Saving..." : "Save"}
          </button>
        </div>
      ...
      {tab === "details" && (
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            ...
            <div>
              <label className="text-[11px] text-white/40 mb-1 block">Trial Message Limit</label>
            ...
            <div className="flex items-center gap-3 pt-2">
              <div className="text-[11px] text-white/40">Trial Used:</div>
              ...
              <div className="text-[11px] text-white/30">{user.trialMessageCount}/{user.trialMessageLimit}</div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-white/10 p-6 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <label className="text-[11px] text-white/40">License Verified</label>
              ...
              {auditLogs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {log.details && (
                      <p className="text-white/20 text-[10px] mt-0.5 truncate">{log.details}</p>
                    )}
                  </div>
                  <span className="text-[10px] text-white/20 shrink-0">{new Date(log.createdAt).toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
