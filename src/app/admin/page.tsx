    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <Link href="/admin/users" className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 text-xs font-medium hover:bg-white/10 transition-colors">
            Manage Users
          </Link>
          <Link href="/admin/support" className="px-4 py-2 rounded-lg bg-[#FDB02F]/10 border border-[#FDB02F]/20 text-[#FDB02F] text-xs font-medium hover:bg-[#FDB02F]/20 transition-colors">
        ...
        return items.length > 0 ? (
          <div className="rounded-2xl bg-[#0D1B4B]/40 border border-orange-400/20 p-5 mb-8">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-white font-semibold text-sm">Action Required</h2>
            </div>
            <div className="flex flex-wrap gap-3">
              {items.map((item) => (
                <Link key={item.label} href={item.href} className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/5 hover:border-white/15 transition-colors">
                  ...
                  <div>
                    <p className="text-white text-xs font-bold">{item.count}</p>
      ...
      {data && (
        <div className="grid md:grid-cols-2 gap-4 mb-8">
          <div className="p-6 rounded-2xl bg-[#0D1B4B]/40 border border-white/10">
            <h2 className="text-white font-semibold text-sm mb-4">Plan Distribution</h2>
            <div className="space-y-3">
              {data.planBreakdown.map((p) => {
                const pct = kpis && kpis.totalUsers > 0 ? (p.count / kpis.totalUsers * 100) : 0;
                return (
                  <div key={p.plan} className="flex items-center gap-3">
                    <span className="text-xs text-white/50 w-24 capitalize">{p.plan}</span>
                    <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-[#FDB02F]/60 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-xs text-white/40 w-16 text-right">{p.count} ({pct.toFixed(0)}%)</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="p-6 rounded-2xl bg-[#0D1B4B]/40 border border-white/10">
            <h2 className="text-white font-semibold text-sm mb-4">Trial & Conversion</h2>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                <p className="text-2xl font-bold text-white">{kpis?.freeUsers || 0}</p>
                <p className="text-[11px] text-white/35 mt-1">Free Users</p>
              </div>
              <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                <p className="text-2xl font-bold text-white">{kpis?.paidUsers || 0}</p>
                <p className="text-[11px] text-white/35 mt-1">Paid Users</p>
              </div>
              <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                <p className="text-2xl font-bold text-[#FDB02F]">{kpis?.conversionRate || 0}%</p>
                <p className="text-[11px] text-white/35 mt-1">Conversion Rate</p>
              </div>
              <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5">
                <p className="text-2xl font-bold text-orange-400">{kpis?.trialExhausted || 0}</p>
                <p className="text-[11px] text-white/35 mt-1">Trial Exhausted</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Quick Links */}
      <h2 className="text-white font-semibold text-sm mb-3">Website Management</h2>
      ...
      <div className="bg-[#0D1B4B]/40 border border-white/10 rounded-2xl p-6">
        <h2 className="text-white font-semibold text-sm mb-4">Managed Pages</h2>
          ...
          {[
            ...
            <Link key={page.slug} href={`/admin/pages?page=${page.slug}`} className="flex items-center justify-between px-4 py-3 rounded-xl hover:bg-white/5 transition-colors group">
              <div className="flex items-center gap-3">
                ...
                <span className="text-white/70 text-sm font-medium">{page.name}</span>
                <span className="text-white/20 text-xs">{page.sections} sections</span>
              </div>
