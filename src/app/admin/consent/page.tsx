      ...
      {showCreate && (
        <div className="rounded-2xl bg-[#0D1B4B]/60 border border-[#FDB02F]/20 p-6 mb-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input placeholder="Version (e.g. 2.0)" value={form.version} onChange={(e) => setForm({ ...form, version: e.target.value })} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg placeholder:text-white/20 focus:outline-none" />
            <input placeholder="Title (e.g. Terms of Service)" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg placeholder:text-white/20 focus:outline-none" />
          </div>
          <textarea placeholder="Full consent text..." value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={8} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-lg placeholder:text-white/20 focus:outline-none resize-none" />
          <div className="flex gap-2">
            <button onClick={handleCreate} className="px-4 py-2 rounded-lg bg-[#FDB02F] text-[#07123A] text-lg font-bold">Publish</button>
          ...
          <div key={v.id} className={`rounded-2xl border p-5 ${v.isActive ? "bg-[#0D1B4B]/60 border-[#FDB02F]/20" : "bg-[#0D1B4B]/20 border-white/5 opacity-60"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-white text-lg font-medium">{v.title} <span className="text-white/30 font-mono">v{v.version}</span></p>
