    ...
    <div className="p-8">
      ...
      {showCreate && (
        <div className="rounded-2xl bg-[#0D1B4B]/60 border border-[#FDB02F]/20 p-6 mb-6 space-y-3">
          <p className="text-white/40 text-sm">Publishing a new version will deactivate all previous versions. Users must re-accept.</p>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/25 text-xs">{v.isActive ? "Active" : "Archived"} • Published {new Date(v.publishedAt).toLocaleDateString()}</p>
                </div>
              </div>
              {v.isActive && <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-400/10 text-green-400 border border-green-400/20">Current</span>}
            </div>
            <p className="text-white/20 text-sm mt-3 line-clamp-2">{v.content}</p>
          </div>
        ))}
      </div>
    </div>
